"""
Production test for Chipotle menu scraping
...
def test_chipotle_menu_scrape():
    """Test scraping Chipotle menu and storing in database"""

    print("=" * 80)
    print("CHIPOTLE MENU SCRAPING TEST - PRODUCTION")
    print("=" * 80)
    ...
    try:
        ...
        try:
            ...
            restaurant_id = f"chipotle_{url.split('//')[-1].split('/')[0].replace('.', '_')}"
        ...
        output_file = f"chipotle_menu_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
